...
namespace TheTaleOfGod
{
    public class Collision
    {
        ...
        public static Rectangle[] CollidingRectangle(Vector2 position, Cell[] cellCheck, int width, int height, out Collider[] colInfo, string closeCollderTag, out Collider[] closeColliders) // need multiple collider objects!!
        {
            List<Rectangle> colliders = null;
            List<Collider> colInfos = null;
            List<Collider> closeColls = null;
            ...
            foreach (var cell in cellCheck)
            {
                if (cell.colliders.Count > 0)
                {
                    foreach (var co in cell.colliders)
                    {
                        if (closeCollderTag == co.tag)
                        {
                            if (closeColls == null)
                            {
                                closeColls = new List<Collider>();
                            }
                            closeColls.Add(co);
                        }
            ...
            {
                closeColliders = closeColls.ToArray();
            }
            if (colliders == null)
            {
                colInfo = null;
        ...
        /// <summary>
        /// moves the object to the edge of the collider
        /// </summary>
        /// <returns></returns>
